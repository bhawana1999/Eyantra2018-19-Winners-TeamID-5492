...
class DroneFly():
    ...
    def __init__(self):
        ...
        # Red
        self.Plant_location[0] = [2.9, 0.2, 25.3]
        # Green 
        self.Plant_location[1] = [-5.5, 2.9, 22]
        # Blue
        self.Plant_location[2] = [5.2, -5.5, 18.8]

        w,h = 3,11
        self.points = [[0.0 for x in range(w)] for y in range(h)]
        self.points[0] = [-1.5,5.78,23]
        self.points[1] = [(self.Plant_location[0][0]+self.points[0][0])/2, (self.Plant_location[0][1]+self.points[0][1])/2, min(self.points[0][2],self.Plant_location[0][2])-0.5]
        # Red
        #self.points[1] = [-5.45, 0.34, 22.8]
        self.points[2] = self.Plant_location[0]

        self.points[3]  = [(self.Plant_location[0][0]+self.Plant_location[1][0])/2, (self.Plant_location[0][1]+self.Plant_location[1][1])/2, min(self.Plant_location[1][2],self.Plant_location[0][2])-0.5]

        # Green 
        #self.points[2] = [-0.15 , -4.3 , 21.8]
        self.points[4] = self.Plant_location[1]

        # self.points[4] = [1.7 , -3.6, 17]
        self.points[5]  = [(self.Plant_location[1][0]+self.Plant_location[2][0])/2, (self.Plant_location[1][1]+self.Plant_location[2][1])/2, min(self.Plant_location[1][2],self.Plant_location[2][2])-0.5]
        # Blue
        #self.points[4] = [3.3,-2.2, 19.1]
        self.points[6] = self.Plant_location[2]

        self.points[8] = [0.06 ,6.2,21]
        self.points[7] = [(self.points[8][0]+self.Plant_location[2][0])/2, (self.points[8][1]+self.Plant_location[2][1])/2, min(self.points[8][2],self.Plant_location[2][2])-0.5]

        #self.points[7] = [0.02,5.78,21.31]
        self.points[9] = [-0.14, 5.6, 25.5]
        #self.points[7] = [0.04 , 5.9, 27.3]
        self.points[10] = [-0.07 , 5.83, 28.5]
        # Position to hold.
    ...
    def position_hold(self):
        ...
        while True:
            ...
            if (time.time()-self.timer > 50) and self.c ==1:
                print ("Stop")
                self.disarm()
                break
            if self.counter == 2 or self.counter == 4 or self.counter == 6:
                if (time.time()-self.time_count > 25) and self.c1 == 0:
                    ...
                    self.c1 = 1
                elif (time.time() - self.time_count1 > 8) and self.c1 == 1:
                    print ("5s done 1")
                    ...
                    self.wp_z = self.points[self.counter][2]
                    self.c1 = 2
                elif (time.time() - self.time_count2 > 8) and self.c1 == 2:
                    print ("5s done 2")
                ...
                elif (time.time() - self.time_count3>8) and self.c1 == 3:
                    print("5s done 3")
            ...
            if self.counter == 2 and self.count_color == 1:
                self.counter = self.counter+1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if (self.counter == 4 ) and self.count_color == 2 - self.point_skipped :
                self.counter = self.counter+1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if (self.counter == 6 ) and self.count_color == 3  - self.point_skipped:
                self.counter = self.counter+1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if (self.counter == 0 or self.counter == 1 or self.counter == 3 or self.counter == 5 or self.counter == 7) and abs(self.drone_x - self.wp_x) < 2 and abs(self.drone_y-self.wp_y)< 2 and abs(self.drone_z - self.wp_z) < 1:
                self.counter = self.counter+1
            ...
            if self.counter == 8 and abs(self.drone_x - self.wp_x) < 2 and abs(self.drone_y-self.wp_y)< 2 and abs(self.drone_z - self.wp_z) < 2:
                self.counter = self.counter + 1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if self.counter == 10 and abs(self.drone_x - self.wp_x) < 0.3 and abs(self.drone_y- self.wp_y)< 0.3 and abs(self.drone_z - self.wp_z) < 0.8 and self.c==1:
                self.c += 1  
                print ("STOP")
                self.disarm()
