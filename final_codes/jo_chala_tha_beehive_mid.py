...
class DroneFly():
    ...
    def __init__(self):
        ...
        # Red
        self.Plant_location[0] = [-5, -5.2, 24.3]
        # Green 
        self.Plant_location[1] = [7.57, 2.7, 20.5]
        # Blue
        self.Plant_location[2] = [-7.1, 5.39, 16.2]

        w,h = 3,10
        self.points = [[0.0 for x in range(w)] for y in range(h)]
        self.points[0] = [-1.5,5.78,23]

        self.points[1] = [(self.Plant_location[0][0]+self.points[0][0])/2, (self.Plant_location[0][1]+self.points[0][1])/2, max(self.points[0][2],self.Plant_location[0][2])]
        # Red
        #self.points[1] = [-5.45, 0.34, 22.8]
        self.points[2] = self.Plant_location[0]

        self.points[3]  = [(self.Plant_location[0][0]+self.Plant_location[1][0])/2, (self.Plant_location[0][1]+self.Plant_location[1][1])/2, max(self.Plant_location[1][2],self.Plant_location[0][2])]
        ...
        self.points[5]  = [(self.Plant_location[1][0]+self.Plant_location[2][0])/2, (self.Plant_location[1][1]+self.Plant_location[2][1])/2, max(self.Plant_location[1][2],self.Plant_location[2][2])]
        # Blue
        #self.points[4] = [3.3,-2.2, 19.1]
        self.points[6] = self.Plant_location[2]

        self.points[7] = [0.02,5.78,21.31]
        self.points[8] = [-0.98, 6.04, 24.63]
        #self.points[7] = [0.04 , 5.9, 27.3]
        self.points[9] = [-0.17 , 5.75, 28]
        # Position to hold.
    ...
    def position_hold(self):
        ...
        while True:
            ...
            if (self.counter == 4 ) and self.count_color == 2 :
                self.counter = self.counter+1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if (self.counter == 6 ) and self.count_color == 3 :
                self.counter = self.counter+1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if (self.counter == 0 or self.counter==1 or self.counter == 3 or self.counter == 5) and abs(self.drone_x - self.wp_x) < 2 and abs(self.drone_y-self.wp_y)< 2 and abs(self.drone_z - self.wp_z) < 1:
                self.counter = self.counter+1
            ...
            if self.counter == 7 and self.c==0:
                print("\n Pollination Done! Pollinated 1 Red Daylily, 1 Green Carnation and 1 Blue Delphinium")
                self.c+=1
            ...
            if self.counter == 7 and abs(self.drone_x - self.wp_x) < 1.5 and abs(self.drone_y-self.wp_y)< 1.5 and abs(self.drone_z - self.wp_z) < 1.5:
                self.counter = self.counter + 1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if self.counter == 8 and abs(self.drone_x - (-0.98)) < 1 and abs(self.drone_y-6.04)< 1 and abs(self.drone_z - 24.5) < 1.2:
                self.counter = self.counter + 1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if self.counter == 9 and abs(self.drone_x - (-0.19)) < 0.8 and abs(self.drone_y-5.6)< 0.8 and abs(self.drone_z - 28) < 1 and self.c==1:
                self.c += 1  
                print ("STOP")
                self.disarm()
