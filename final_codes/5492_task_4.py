...
class DroneFly():
    ...
    def __init__(self):
        ...
        # Red
        self.Plant_location[0] = [-5.45, 0.34, 22.8]
        # Green 
        self.Plant_location[1] = [-0.15 , -4.3 , 21.8]
        # Blue
        self.Plant_location[2] = [3.3,-2.2, 19.1]

        w1,h1 = 3,6
        ##List of all the coordinates needed to be traversed 
        self.waypoints = [[0.0 for x in range(w1)] for y in range(h1)] 
        self.waypoints[0] = self.Plant_location[0]
        self.waypoints[1] = self.Plant_location[1]
        self.waypoints[2] = self.Plant_location[2]

        # these waypoints are used for landing the drone on the beehive
        self.waypoints[3] = [0.02,5.78,21.31]
        self.waypoints[4] = [0.09, 5.47, 24.5]
        self.waypoints[5] = [-0.05 , 6.2, 27.2]
        


        # to attain an initial height.
        self.wp_x = -1.5
        self.wp_y = 5.78
        self.wp_z = 19.5
    ...
    def position_hold(self):
        ...
        while True:
            ...
            if (self.counter == -1 ) and abs(self.drone_x - self.wp_x) < self.thresh and abs(self.drone_y-self.wp_y)< self.thresh and abs(self.drone_z - self.wp_z) < self.threshZ:
                
                self.counter = self.counter+1
                self.wp_x = self.waypoints[self.counter][0]
                self.wp_y = self.waypoints[self.counter][1]
                self.wp_z = self.waypoints[self.counter][2]


            if (self.counter == 0 or self.counter == 1 or self.counter == 2 ) and self.count_color == self.counter+1:
            ...
            if self.counter == 3 and self.c==0:
                print("\n Pollination Done! Pollinated 1 Red Daylily, 1 Green Carnation and 1 Blue Delphinium")
                self.c+=1
                

            ## Different thresholds are given for the these points to facilitate landing of drone at the beehive
            if (self.counter == 3 or self.counter == 4) and abs(self.drone_x - self.wp_x) < 1.2 and abs(self.drone_y-self.wp_y)< 1.2 and abs(self.drone_z - self.wp_z) < 1.5:
                self.counter = self.counter + 1
                self.wp_x = self.waypoints[self.counter][0]
                self.wp_y = self.waypoints[self.counter][1]
                self.wp_z = self.waypoints[self.counter][2]
            ...
            if self.counter == 5 and abs(self.drone_x - self.wp_x) < 0.8 and abs(self.drone_y- self.wp_y)< 0.8 and abs(self.drone_z - self.wp_z) < 1 and self.c==1:
                self.c += 1  
                print ("STOP")
                self.disarm()
