...
class DroneFly():
    ...
    def __init__(self):
        ...
        # Red
        self.Plant_location[0] = [-4.6, 0.65, 25]
        # Green 
        self.Plant_location[1] = [-0.28, -5.1, 24]
        # Blue
        self.Plant_location[2] = [4.8, -0.4, 22.8]

        self.Plant_location[3] = [-1.18, 0.11, 22.8]
        ...
        self.points[2] = [(self.Plant_location[0][0]+self.Plant_location[1][0])/2, (self.Plant_location[0][1]+self.Plant_location[1][1])/2, max(self.Plant_location[1][2],self.Plant_location[0][2])]

        # Green 
        self.points[3] = self.Plant_location[1]

        self.points[4] = [(self.Plant_location[1][0]+self.Plant_location[2][0])/2, (self.Plant_location[1][1]+self.Plant_location[2][1])/2, max(self.Plant_location[1][2],self.Plant_location[2][2])]

        # Blue
        #self.points[5] = [2, -5.5, 18.7]
        self.points[5] = self.Plant_location[2]

        self.points[6] = [(self.Plant_location[3][0]+self.Plant_location[2][0])/2, (self.Plant_location[3][1]+self.Plant_location[2][1])/2, max(self.Plant_location[3][2],self.Plant_location[2][2])]

        #Red
        self.points[7] = self.Plant_location[3]


        self.points[8] = [0.02,5.78,21.31]
        self.points[9] = [-0.98, 6.04, 24.63]
        #self.points[7] = [0.04 , 5.9, 27.3]
        self.points[10] = [-0.17 , 5.75, 28]
    ...
    def position_hold(self):
        ...
        while True:
            ...
            if self.counter == 8 and abs(self.drone_x - self.wp_x) < 2.5 and abs(self.drone_y-self.wp_y)< 2.5 and abs(self.drone_z - self.wp_z) < 2.5:
                self.counter = self.counter + 1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if self.counter == 9 and abs(self.drone_x - 0.09) < 1.5 and abs(self.drone_y-5.3)< 1.5 and abs(self.drone_z - 24.5) < 1.2:
                self.counter = self.counter + 1
                self.wp_x = self.points[self.counter][0]
                self.wp_y = self.points[self.counter][1]
                self.wp_z = self.points[self.counter][2]
            ...
            if self.counter == 10 and abs(self.drone_x - (-0.19))< 0.8 and abs(self.drone_y-5.6)< 0.8 and abs(self.drone_z - 28) < 1 and self.c == 1 :
                self.c += 1  
                print ("STOP")
                self.disarm()
