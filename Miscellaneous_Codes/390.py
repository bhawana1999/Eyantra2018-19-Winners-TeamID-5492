...
class DroneFly():
    ...
    def position_hold(self):
        ...
        while True:
            ...
            self.cmd.rcThrottle = self.limit(throt_value, 1520, 1280)
    ...
    def pid_throt(self):
        ...
        PID = self.kp_throt * error * 0.80 + self.kd_throt * dErr / self.loop_time + self.ki_throt * self.errorsumZ * self.loop_time * 0.80
    ...
    def move(self):
        ...
        if self.wp_x == -5.63 and self.wp_y == -5.63:
            if abs(self.drone_z-self.wp_z)<1.0 and abs(self.drone_y-self.wp_y)<0.2 and abs(self.drone_x-self.wp_x)<0.2:
                if(self.i<10):
                    self.i+=1
                    return
                self.wp_x=5.57
        ...
        elif self.wp_x == 5.57 and self.wp_y == -5.63:
            if abs(self.drone_z-self.wp_z)<1.0 and abs(self.drone_y-self.wp_y)<0.2 and abs(self.drone_x-self.wp_x)<0.2:
                if(self.i<10):
                    self.i+=1
                    return
        ...
        elif self.wp_x == 5.55 and self.wp_y == 5.54:
            if abs(self.drone_z-self.wp_z)<1.0 and abs(self.drone_y-self.wp_y)<0.2 and abs(self.drone_x-self.wp_x)<0.2:
                if(self.i<10):
                    self.i+=1
                    return
		...
		self.i=0
	elif self.wp_x == -5.6 and self.wp_y == 5.54:
            if abs(self.drone_z-self.wp_z)<1.0 and abs(self.drone_y-self.wp_y)<0.2 and abs(self.drone_x-self.wp_x)<0.2:
                if(self.i<10):
                    self.i+=1
                    return
		...
		self.i=0
	elif self.wp_x == 0.00 and self.wp_y == 0.00:
            return
	...
	self.pubr=rospy.Publisher('/red', Int32,queue_size=10)
	self.pubb=rospy.Publisher('/blue', Int32,queue_size=10)
	self.pubg=rospy.Publisher('/green', Int32,queue_size=10)
    ...
    def image_callback(self, msg):
        ...
        img1 = self.ros_bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        ...
        # setting red blue and green thresholds
	...
	#creating masks for detecting red blue and green
        maskr = cv2.inRange(hsv, lower_red, upper_red)
        maskb = cv2.inRange(hsv, lower_blue, upper_blue)
        maskg = cv2.inRange(hsv, lower_green, upper_green)
	
	#finding number of contours int the mask frames
        ...
        imG, contoursG, hierarchyG = cv2.findContours(maskg, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(img, contoursR, -1, (0, 255, 0), 4)
	# calculating the number of red, green and blue contours
	for cnt in contoursG:
        	self.numG += 1
	
	self.pubg.publish(self.numG)
        rospy.sleep(0.1)        
	...
	self.pubr.publish(self.numR)
	rospy.sleep(0.1)
        
	for cnt in contoursB:
        	self.numB += 1
	
	self.pubb.publish(self.numB)
	rospy.sleep(0.1)
